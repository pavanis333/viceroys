        ...
        <div className="flashcard">
          ...
          >
            <div className="flashcard-front">
              ...
              <div style={{fontSize: '1.2rem', color: '#8b0000', marginBottom: '15px'}}>
                {viceroy.period}
              </div>
            ...
            <div className="flashcard-back">
              <div style={{textAlign: 'left', width: '100%'}}>
                ...
                {viceroy.keyReforms && viceroy.keyReforms.length > 0 && (
                  <div className="viceroy-info" style={{marginTop: '10px'}}><strong>Key Reforms:</strong> {viceroy.keyReforms.join('; ')}</div>
                )}
                {viceroy.specialNote && (
                  <div className="viceroy-info" style={{marginTop: '10px', fontStyle: 'italic', fontSize: '0.85rem'}}><strong>Note:</strong> {viceroy.specialNote}</div>
                )}
              </div>
            </div>
          </div>
    ...
    return (
      <div>
        ...
        <h2 style={{marginBottom: '20px', color: '#1a3a0f'}}>
          {viceroys.length} Viceroy(s) Found
        </h2>

        <div className="viceroys-grid">
          {viceroys.map(viceroy => (
            <div key={viceroy.id} className="viceroy-card">
              <h3>{viceroy.name}</h3>
              <div className="tags">
                <span className="tag">{viceroy.period}</span>
                <span className={`tag ${viceroy.importance === 'HIGH' ? 'high' : ''}`}>
                  {viceroy.importance}
                </span>
              </div>
              <div className="viceroy-info"><strong>Key Events:</strong></div>
              <ul style={{margin: '5px 0', paddingLeft: '20px', fontSize: '0.85rem'}}>
                {viceroy.keyEvents?.slice(0, 3).map((event, i) => <li key={i}>{event}</li>)}
              </ul>
              {viceroy.keyReforms && viceroy.keyReforms.length > 0 && (
                <div className="viceroy-info" style={{marginTop: '8px', fontSize: '0.85rem', fontStyle: 'italic'}}>
                  <strong>Reforms:</strong> {viceroy.keyReforms[0]}
                </div>
              )}
            </div>
          ))}
